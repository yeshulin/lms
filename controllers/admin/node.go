...
func (this *NodeController) Get() {
	...
	if search != "" {
		where = where + " and (name like '%" + search + "%' or title like '%" + search + "%')"
	}
	qb, _ := orm.NewQueryBuilder("mysql")

	// 构建查询对象
	qb.Select("id,name,title,status,sort,pid,type").
		From("node").
		Where(where).
		OrderBy(sort).Desc().
...
func (this *NodeController) AddPost() {
	...
	node.Title = this.GetString("title")
	node.Status = status
	node.Remark = this.GetString("remark")
	node.Sort = sort
	node.Pid = pid
	node.Type = this.GetString("type")
